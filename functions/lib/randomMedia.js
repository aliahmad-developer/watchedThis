    ...
    for (let i = 0; i < retries; i++) {
        try {
            ...
            if (res.status === 401 || res.status === 403) {
                throw new Error(`TMDB auth failed: ${res.status} — check TMDB_API_KEY on Cloud Run`);
            }
        }
        catch (err) {
            console.error(`[randomMedia] fetch attempt ${i + 1} failed:`, err === null || err === void 0 ? void 0 : err.message);
            if (i === retries - 1)
                throw err;
...
// ── Main ──────────────────────────────────────────────────────────────────────
async function getRandomMedia(seenIds = new Set(), count = 1) {
    var _a, _b;
    // Fail fast if key is missing — don't waste time on doomed requests
    if (!TMDB_API_KEY) {
        throw new Error("TMDB_API_KEY is not set — add it to Cloud Run environment variables");
    }
    const results = [];
    ...
    for (let i = 0; i < maxAttempts && results.length < count; i++) {
        ...
        const url = `${TMDB_BASE}/discover/${media_type}?api_key=${TMDB_API_KEY}` +
            `&language=en-US&sort_by=${sort}&${era.dateParam}=${year}` +
            `&with_genres=${genre}&vote_count.gte=${era.minVotes}&page=${page}`;
        try {
            const res = await fetchWithRetry(url);
            const data = await res.json();
            const valid = ((_a = data.results) !== null && _a !== void 0 ? _a : []).filter((item) => {
                var _a;
            ...
            console.error(`[randomMedia] attempt ${i + 1} error:`, err === null || err === void 0 ? void 0 : err.message);
            // If it's an auth error, stop immediately
            if ((_b = err === null || err === void 0 ? void 0 : err.message) === null || _b === void 0 ? void 0 : _b.includes("TMDB auth failed"))
                throw err;
...
//# sourceMappingURL=randomMedia.js.map