...
export default function SpecificRandomMedia({ media_type, media_name_slug, id }: Props) {
  ...
  useEffect(() => {
    ...
    // AbortController so navigating away mid-fetch doesn't cause state updates
    // on an unmounted component instance.
    ...
    const fetchData = async () => {
      try {
        ...
        setData(json);
        // ↑ No router.replace here — slug normalization is handled server-side
        //   in page.tsx via redirect(). Calling router.replace() from a fully
        //   rendered client component corrupts the router history stack and
        //   causes subsequent router.push() calls (e.g. the Random nav button)
  ...
  }, [media_type, media_name_slug, id]);

  // Once both data loaded and min delay elapsed → transition out the loader
  useEffect(() => {
  ...
  // Re-sync the loader state machine if the browser throttled our timers
  ...
  }, [loading, minDelayDone, showLoader]);
