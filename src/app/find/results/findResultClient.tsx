...
// ── Ambient color helpers (same as Desc.tsx) ──────────────────────────────────
...
export default function FindResultsPage() {
  ...
  useEffect(() => { fetchResults(1, false); }, [fetchResults]);
      ...
      <div className="w-full max-w-7xl mx-auto px-3 sm:px-6 lg:px-10 py-4 sm:py-6 lg:py-8 space-y-4 sm:space-y-5">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          >
            ...
            <span className="hidden sm:inline text-light-btn-text dark:text-shadow-dark-btn-text hover:text-light-btn-hover-text hover:dark:text-dark-btn-hover-text transition-colors">Back to filters</span>
          </button>
          <div className="flex-1 min-w-0">
            <h1 className="text-sm sm:text-lg lg:text-xl font-bold capitalize truncate">
              {mediaType === "tv" ? "TV Shows" : "Movies"}
              {keyword && <span className="text-light-accent dark:text-dark-accent"> — "{keyword}"</span>}
            </h1>
        ...
        {(genreNames.length > 0 || minYear || minRating) && (
          <div className="flex flex-wrap gap-1.5 sm:gap-2">
            {genreNames.map(name => (
              <span key={name} className="text-[10px] sm:text-xs px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-full bg-light-accent/10 dark:bg-dark-accent/10 text-light-accent dark:text-dark-accent border border-light-accent/20 dark:border-dark-accent/20">
                {name}
              </span>
            ))}
            {(minYear || maxYear) && (
        ...
        <div className="space-y-3 sm:space-y-4">
          ...
          {error && (
            <div className="text-center py-16 space-y-3">
              ...
              <button onClick={() => fetchResults(1, false)}
          ...
          {!loading && !error && results.length === 0 && (
            <div className="flex flex-col items-center justify-center py-20 gap-3 text-center">
              <FontAwesomeIcon icon={faSearch} className="h-10 sm:h-12 text-light-accent dark:text-dark-accent opacity-30" />
              <p className="text-sm sm:text-base font-semibold">No results found</p>
              <p className="text-xs sm:text-sm text-light-secondary-text dark:text-dark-secondary-text">Try adjusting your filters.</p>
          ...
          {results.map(item => <ResultCard key={`${item.id}-${item.media_type}`} item={item} />)}
          {loading && results.length > 0 &&
...
function ResultCard({ item }: { item: MediaResult }) {
  ...
  // Same gradient stack as Desc.tsx
  ...
  // Centre vignette so title always pops
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full aspect-4/3 sm:aspect-16/6 lg:aspect-16/5 overflow-hidden">
        ...
        {/* Ambient tint over whole image */}
        <div className="absolute inset-0 transition-all duration-700"
          style={{ backgroundColor: fullTint }} />

        {/* Bottom fade into info bar */}
        <div className="absolute inset-0 transition-all duration-700"
        ...
        {/* Centre darkening for title readability */}
      ...
      {/* Info bar — uses solidColor as bg so it matches the ambient palette */}
      <div className="px-3 sm:px-4 py-2 sm:py-3 space-y-1 transition-all duration-700"
