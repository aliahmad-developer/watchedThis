...
export function deriveGenres(item: MediaItem): string[] {
  ...
  if (item.genre_ids?.length)
    return item.genre_ids.map((id) => TMDB_GENRES[id]).filter(Boolean) as string[];
  return [];
}
...
export function scoreItem(
  ...
  for (const query of recentSearches) {
    ...
    if (fuzzy(title, query))
      add("search_title", W.SEARCH_TITLE + bonus);
...
export function deriveTasteProfile(
  ...
  return {
    ...
    activityScore:   favouriteIds.length * 3 + libraryIds.length * 2 + clickLog.length,
  };
}