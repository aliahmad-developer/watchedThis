...
type CameraMode = "idle" | "chooser" | "live" | "uploading";
  ...
  return (
    <>
      {/* ── Sticky Camera Button ──
          Positioned bottom-6 LEFT instead of right to avoid overlapping
          with the scroll-to-top button which sits bottom-6 right.
          On pages without a scroll-to-top button it still looks natural. */}
      <button
            ...
            {mode === "chooser" && (
              <div className="p-6 flex flex-col gap-4">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={closeModal} className="bg-transparent text-light-secondary-text dark:text-dark-secondary-text hover:text-light-body-text dark:hover:text-dark-body-text transition-colors">
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {mode === "live" && (
              <div className="flex flex-col">
                ...
                <div className="flex items-center justify-between p-4 gap-3">
                  <button onClick={closeModal} className="bg-transparent text-xs text-light-secondary-text dark:text-dark-secondary-text hover:text-light-body-text dark:hover:text-dark-body-text transition-colors">
                    Cancel
                  </button>
                  <button
                  ...
                  <button onClick={() => { stopCamera(); setMode("chooser"); }} className="bg-transparent text-xs text-light-secondary-text dark:text-dark-secondary-text hover:text-light-body-text dark:hover:text-dark-body-text transition-colors">
                    Upload instead
                  </button>
                </div>
              </div>
            ...
            {mode === "uploading" && (
              <div className="p-10 flex flex-col items-center gap-4">
                <div className="flex gap-1.5">
                  {[0, 1, 2].map((i) => (
                    <div key={i} className="w-2.5 h-2.5 rounded-full bg-light-accent dark:bg-dark-accent animate-bounce" style={{ animationDelay: `${i * 0.15}s` }} />
                  ))}
                </div>
